...
from ..networks import wif_prefix_for_netcode, address_prefix_for_netcode
...
class LazySecretExponentDB(object):
    ...
    of secret exponents.
    """
    def __init__(self, wif_iterable, secret_exponent_db_cache, network='BTC'):
    ...
    def get(self, v):
        ...
        for wif in self.wif_iterable:
            secret_exponent = wif_to_secret_exponent(wif, allowable_wif_prefixes=[wif_prefix_for_netcode(self.network)])
...
def create_tx(spendables, payables, fee="standard", lock_time=0, version=1, network='BTC'):
    """
    ...
    for payable in payables:
        if len(payable) == 2:
            ...
            coin_value = 0
        script = standard_tx_out_script(bitcoin_address, network=network)
...
def sign_tx(tx, wifs=[], secret_exponent_db={}, network='BTC'):
    """
    ...
    """
    tx.sign(LazySecretExponentDB(wifs, secret_exponent_db, network=network))


def create_signed_tx(spendables, payables, wifs=[], fee="standard",
                     lock_time=0, version=1, secret_exponent_db={}, network='BTC'):
    """
    ...
    sign_tx(tx, wifs=wifs, secret_exponent_db=secret_exponent_db, network=network)
    for idx, tx_out in enumerate(tx.txs_in):
        if not tx.is_signature_ok(idx):
            raise SecretExponentMissing("failed to sign spendable for %s" %
                                        tx.unspents[idx].bitcoin_address(netcode=network))
    return tx
